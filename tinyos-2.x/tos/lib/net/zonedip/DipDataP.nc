...
implementation {
  ...
  command error_t DipDecision.send() {
    ...
    data = call ZoneDisseminationValue.get(key);
  ...
  event void DataReceive.receive(void* payload, uint8_t len) {
    ...
    if(myVer < msgVer) {
      call ZoneDisseminationUpdate.change(key, (dip_data_t*)ddmsg->data);
  ...
  event void ZoneDisseminationValue.changed(dip_key_t key) {  }
  
 default command const dip_data_t* ZoneDisseminationValue.get(dip_key_t key) {
   return NULL;
 }

 default command void ZoneDisseminationUpdate.change(dip_key_t key, dip_data_t* val) { }
