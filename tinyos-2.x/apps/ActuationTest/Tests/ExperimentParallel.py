...
import subprocess
from plot_grant_release import *
...
class Simulation:
    ...
    def GenerateResults(self, trytime, holdfactor, gridsize, densities, runs,
        ...
        plotbase = 'plots/try' + trytime + '/hold' + holdfactor 
        plotbase += '/' + transport
        plotbase += '/parallel_' + str(gridsize) + '_d'
        sim_stats = {}
        tmsg_stats = {}
        print 'Generating Results grid: ', gridsize, 'x', gridsize
        for d in range(len(densities)):
            extlogbase = logbase + str(d) + '_'
            extplotbase = plotbase + str(d) + '_'
            local_stats = []
            local_tmsg = []
            for i in range(runs):
                logfilename = extlogbase + str(i) + '.txt'
                plotfilename = extplotbase + str(i) + '.pdf'
                print '[', densities[d], i, ']'
                self.lf.ParseFile(logfilename)
                (stats, tmsg) = self.lf.GetStats()
                xpairs = [(nstat['granttimes'][0], nstat['releasetimes'][0])
                        for nstat in stats.itervalues()]
                if self.verbose:
                    print xpairs
                plot_point_list(xpairs, plotfilename);
    ...
    def PlotAppLatency(self, trytime, holdfactor, grids, stats, runs,
        ...
        plotfilename += '/' + transport + '/app_latency.txt'
    ...
    def PlotParallel(self, trytime, holdfactor, gridsize, densities,
            runs, transport):
        logbase = 'logs/try' + trytime + '/hold' + holdfactor 
        logbase += '/' + transport
        logbase += '/log_' + str(gridsize) + '_d'
        plotbase = 'plots/try' + trytime + '/hold' + holdfactor
        plotbase += '/' + transport + '/parallel_d'
        print '# Plotting parallel operation: ', gridsize, 'x', gridsize
        for d in range(len(densities)):
            extlogbase = logbase + str(d) + '_'
            extplotbase = plotbase + str(d) + '_'
            for i in range(runs):
                logfilename = extlogbase + str(i) + '.txt'
                plotfilename = extplotbase + str(i) + '.pdf'
                plot_grant_release(logfilename, plotfilename, self.verbose)
    ...
    def PlotRTvsDensity(self, trytime, holdfactor, grids, stats, runs,
            transport):
        plotfilename = 'plots/try' + trytime + '/hold' + holdfactor
        plotfilename += '/' + transport + '/rt_density.txt'
        plotfile = open(plotfilename, 'w')
    ...
    def PlotConflictsVsDensity(self, trytime, holdfactor, grids, stats, runs,
            transport):
        plotfilename = 'plots/try' + trytime + '/hold' + holdfactor
        plotfilename += '/' + transport + '/conflicts_density.txt'
        plotfile = open(plotfilename, 'w')
    ...
    def PlotPkterrVsDensity(self, trytime, holdfactor, grids, stats, runs,
        ...
        #          'rx' : rxcount }
        plotfilename = 'plots/try' + trytime + '/hold' + holdfactor
        plotfilename += '/' + transport + '/pkterr_density.txt'
        plotfile = open(plotfilename, 'w')
    ...
    def PlotRT(self, trytime, holdfactor, gridsize, densities, sim_stats,
            runs, transport):
        plotfilename = 'plots/try' + trytime + '/hold' + holdfactor
        plotfilename += '/' + transport + '/rt_data_' + str(gridsize) + '.txt'
        plotfile = open(plotfilename, 'w')
        ...
        for d in range(len(densities)):
            ...
            for nodeid in range(gridsize*gridsize):
                values = []
                for r in sim_stats[d]:
                    values.extend(r[nodeid]['rt'])
                # Calculate mean RT per iteration and stddev for this node
                rt_mean = mean(values)
                rt_sd = sd(values)
                writestr = str(nodeid) + '\t' + str(rt_mean)
                writestr += '\t' + str(rt_sd)
                if self.verbose:
                    print writestr
                plotfile.write(writestr + '\n')
    ...
    def PlotMPCS(self, trytime, holdfactor, gridsize, densities, sim_stats, runs, transport):
        plotfilename = 'plots/try' + trytime + '/hold' + holdfactor
        plotfilename += '/' + transport + '/data_' + str(gridsize) + '.txt'
        plotfile = open(plotfilename, 'w')
        ...
        for d in range(len(densities)):
            ...
            for nodeid in range(gridsize*gridsize):
                values = []
                for r in sim_stats[d]:
                    values.extend(r[nodeid]['mpcs'])
                # Calculate mean RT per iteration and stddev for this node
                mpcs_mean = mean(values)
                mpcs_sd = sd(values)
    ...
    def PlotMPCSvsDensity(self, trytime, holdfactor, grids, stats, runs,
            transport):
        plotfilename = 'plots/try' + trytime + '/hold' + holdfactor
        plotfilename += '/' + transport + '/mpcs_density.txt'
        plotfile = open(plotfilename, 'w')
...
def __test(trytime, holdfactor, verbose, runs, transport, load, gain=None):
    ...
    #sim.Run(trytime, holdfactor, 3, grids[3], runs, transport, load, gain)
    #sim.Run(trytime, holdfactor, 5, grids[5], runs, transport, load, gain)
    #sim.Run(trytime, holdfactor, 7, grids[7], runs, transport, load, gain)
    #sim.Run(trytime, holdfactor, 10, grids[10], runs, transport, load, gain)
    # Now parse the log files
    if 3 in grids:
        (stats[3], tmsg[3]) = sim.GenerateResults(trytime, holdfactor, 3, grids[3], runs, transport)
    if 5 in grids:
        (stats[5], tmsg[5]) = sim.GenerateResults(trytime, holdfactor, 5, grids[5], runs, transport)
    if 7 in grids:
        (stats[7], tmsg[7]) = sim.GenerateResults(trytime, holdfactor, 7, grids[7], runs, transport)
    if 10 in grids:
        (stats[10], tmsg[10]) = sim.GenerateResults(trytime, holdfactor, 10, grids[10], runs, transport)
    # Now plot the results
    #if 3 in grids:
    #    sim.PlotParallel(trytime, holdfactor, 3, grids[3], runs, transport)
    #if 5 in grids:
    #    sim.PlotParallel(trytime, holdfactor, 5, grids[5], runs, transport)
    #if 7 in grids:
    #    sim.PlotParallel(trytime, holdfactor, 7, grids[7], runs, transport)
    #if 10 in grids:
    #    sim.PlotParallel(trytime, holdfactor, 10, grids[10], runs, transport)
    if 3 in grids:
...
if (__name__ == '__main__'):
    ...
    for o, a in opts:
        if o in ("-v", "--verbose"):
            ...
            list_errors = True
    # Load: [1, 3]
    __test('_static', '_relax', verbose, 20, 'unreliable', 1)
    # Load: [15,25]
    #__test('_static', '_relax', verbose, 1, 'unreliable', 2)
    # Load: [60,180]
    #__test('_static', '_relax', verbose, 1, 'unreliable', 3)
    # Load: [300,600]
    #__test('_static', '_relax', verbose, 1, 'unreliable', 4)
    # Load: [1, 3]
    __test('_static', '_strict', verbose, 20, 'unreliable', 1)
    # Load: [15,25]
    #__test('_static', '_strict', verbose, 1, 'unreliable', 2)
    # Load: [60,180]
    #__test('_static', '_strict', verbose, 1, 'unreliable', 3)
    # Load: [300,600]
    #__test('_static', '_strict', verbose, 1, 'unreliable', 4)
