...
module DipLogicP {
  ...
  uses interface ZoneDisseminationUpdate<dip_data_t> as VersionUpdate;
...
implementation {
  ...
  dip_estimate_t estimates[NUM_NODES];
  ...
  command error_t Init.init() {
    ...
    DIP_DATA_ESTIMATE = getDataEstimate(NUM_NODES);
    DIP_MAX_ESTIMATE = getMaxEstimate(NUM_NODES);
    DIP_VECTOR_ESTIMATE = DIP_DATA_ESTIMATE - 1;
    totalPossible = call DipEstimates.estimateToHashlength(0);
    dbg("DipLogicP", "Real Total: %u, Dip Total: %u\n", NUM_NODES, totalPossible);
    if(totalPossible < NUM_NODES) {
      DIP_DATA_ESTIMATE++;
      DIP_MAX_ESTIMATE++;
      DIP_VECTOR_ESTIMATE++;
      totalPossible = call DipEstimates.estimateToHashlength(0);
  ...
  command dip_hashlen_t DipEstimates.estimateToHashlength(dip_estimate_t est) {
    ...
    if(val > NUM_NODES) { // bring length back down if over NUM_NODES
      val = NUM_NODES;
    }
